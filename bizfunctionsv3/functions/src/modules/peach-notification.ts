  ...
  async (request, response) => {
        ...
        await fs.runTransaction(function(transaction) {
          return transaction.get(batchRef).then(async function(batchDocument) {
            if (!batchDocument.exists) {
              throw new Error(
                `Payment Batch Document does not exist! ${bidDocumentId}`
              );
            }
            ...
            for (const key of keys) {
              ...
              if (mbid.exists) {
                ...
                console.log(
                  `ERROR: Original invoice bid not found. Cannot settle: ${key}`
                );
                throw new Error(
                  `InvoiceBid not found for settlement, documentId: ${key}`
                );
              }
            }
    ...
    async function createSettlement(bid, documentId) {
      const settlement = {
        ...
        invoiceBid: `resource:com.oneconnect.biz.InvoiceBid#${
          bid.invoiceBidId
        }`,
