...
export const queryOffers = functions
  ...
  .https.onRequest(async (request, response) => {
    ...
    let open: Boolean = request.body.open;
    if (!open) {
      open = true;
    }
    const result = {
    ...
    async function getOpenOffers() {
      ...
      const msg = `offers query, limit = ${limit}, where isOpen = true, where endTime > ${new Date().toISOString()}, found ${
    ...
    async function getClosedOffers() {
      ...
      const msg4 = `offers query, limit = ${limit}, where isOpen = false, orderBy endTime desc, found ${
        qs4.docs.length
      } elapsed ${end4 - start} milliseconds`;
