...
export async function createWallet(sourceSeed, id, type, debug) {
  ...
  console.log(
    "####### hooking up with Stellar to generate new keys, date: " +
      dateRegistered
  );
    ...
    requestor.get(
      ...
      async function(error, mResponse, body) {
        ...
        if (mResponse.statusCode === 200) {
          console.log(
            "### MAJOR SUCCESS!!! ### test wallet has 10,000 XLM on Stellar. ####"
          );
          wallet.success = true;
          ...
          console.log(error);
          console.log(
            "wallet failed, response code from Stellar: " + mResponse.statusCode
          );
          return handleError();
        }
      }
